...
- [ ] **Task 13: Python dependencies verification** (AC: 23)
  - [ ] Document manual step: Test bcrypt import: `python3.11 -c "from passlib.hash import bcrypt; print('OK')"`
  - [ ] If bcrypt import fails, document troubleshooting: install build dependencies, reinstall passlib
  - [ ] Document verification: Should print "OK" without errors
...
- passlib[bcrypt] 1.7.4 requires python3.11-dev for compilation
- bcrypt uses Rust backend, requires system compiler
...
- Must use bcrypt for password hashing (verification in AC 23)
- Requires python3.11-dev for bcrypt compilation
- Verification command: `python3.11 -c "from passlib.hash import bcrypt; print('OK')"`
...
If bcrypt import fails:
```bash
# Install build dependencies
sudo apt install build-essential python3.11-dev libffi-dev

# Reinstall passlib with bcrypt
python3.11 -m pip install --upgrade --force-reinstall 'passlib[bcrypt]==1.7.4'
```
...
14. **bcrypt Verified**: passlib[bcrypt]==1.7.4 installed and functional via pip
