...
- [x] **Task 9: Create dashboard.sh script** (AC: 21, 22)
  ...
  - [x] Output in Norwegian: "Tjeneste status: Kjører ✓", "Diskplass: 5.2GB / 20GB (26%)", "I dag aktivitet: 12 videoer, 45 minutter"
...
- [x] **Task 10: Document weekly maintenance checklist** (AC: 24)
  ...
  - [x] Weekly checklist (Norwegian):
    - [x] Kjør check-health.sh (run health check)
...
3. Backup current database: `app.db` → `app.db.before-restore`
4. Copy from backup: `backups/$BACKUP_FILE` → `app.db`
...
1. Service Status (Norwegian):
   ...
   - Output: "Tjeneste status: Kjører ✓" or "Tjeneste status: Stoppet ✗"

2. Disk Space (Norwegian, AC 23):
   - Check disk space on /opt/youtube-viewer
   - Warn if <20% free (alert threshold)
   - Output: "Diskplass: 15.2GB / 20GB (76% brukt) ⚠️ ADVARSEL: <20% ledig"
...
4. Today's Activity (AC 21):
   ...
   - Output: "📺 I dag aktivitet:\n  Videoer sett: 12\n  Total tid: 45 minutter"

5. Recent Errors:
   - Count errors in last hour from journalctl
   - Output: "⚠️ Siste feil (siste time):\n  ✅ Ingen feil" or "⚠️ 5 feil (sjekk logger)"
...
- [ ] Kjør check-health.sh (Run health check script)
...
│   ├── scripts/
│   │   ├── deploy.sh             # Story 5.3 - deployment automation
│   │   ├── backup.sh             # THIS STORY - database backup
│   │   ├── restore.sh            # THIS STORY - database restore
│   │   ├── check-health.sh       # THIS STORY - weekly health check
│   │   ├── dashboard.sh          # THIS STORY - real-time dashboard
│   │   └── README.md             # Updated with new scripts usage
│   ├── backend/
│   │   ├── main.py               # Update for JSON logging
│   │   └── logging_config.py     # THIS STORY - structured logging (optional)
│   └── tests/
│       └── backend/
├── data/
...
├── backups/                      # Backup directory (700 permissions)
│   ├── app-20251112-020000.db   # Daily backups (7 days retention)
│   └── app-20251113-020000.db
└── .env                          # Environment variables

/var/log/youtube-viewer/          # ALL application logs (Linux FHS standard, 755 permissions)
├── app.log                       # Main application log (JSON format, 640 permissions)
├── backups.log                   # Backup script log (640 permissions)
└── deployments.log               # Deployment log from Story 5.3 (640 permissions)

/etc/logrotate.d/
└── youtube-viewer                # THIS STORY - logrotate configuration

/etc/systemd/system/
├── youtube-viewer.service        # Story 5.2 - main service
├── youtube-viewer-backup.service # THIS STORY - backup service
└── youtube-viewer-backup.timer   # THIS STORY - backup timer
```
...
- Health endpoint enhanced with ISO 8601 UTC timestamp (AC 1-2) ✅
- JSON structured logging with development fallback (AC 3-5) ✅
- Logrotate configuration with 7-day retention (AC 6-8) ✅
- Backup automation with WAL checkpoint and retention (AC 9-16) ✅
- Restore procedure with integrity verification and rollback (AC 17-19) ✅
- Comprehensive health check script with Norwegian output (AC 20, 22, 23) ✅
- Real-time dashboard with today's activity tracking (AC 21, 22) ✅
- Weekly maintenance checklist in Norwegian (AC 24) ✅
...
|----|----------------|-----------------|--------|
| 1-2 | backend/main.py:180-191 | tests/backend/test_health.py:24-39 PASS | ✅ |
| 3-5 | backend/logging_config.py (104 lines) | Manual (log file creation) | ✅ |
| 6-8 | scripts/logrotate-youtube-viewer | Manual (dry-run test) | ✅ |
| 9-13 | scripts/backup.sh (140 lines) | Manual (execution test) | ✅ |
| 14-16 | systemd service + timer | Manual (systemctl verification) | ✅ |
| 17-19 | scripts/restore.sh (217 lines) | Manual (restore test) | ✅ |
| 20 | scripts/check-health.sh (259 lines) | Code review + manual | ✅ |
| 21 | scripts/dashboard.sh (214 lines) | Code review + manual | ✅ |
| 22 | Norwegian output in monitoring scripts | Code review | ✅ |
| 23 | Alert thresholds documented | Code review + README | ✅ |
| 24 | Weekly checklist in README | Documentation review | ✅ |
...
- Manual execution testing (documented in Dev Agent Record) ✅
- Integration testing (documented as tested) ✅

**Test Coverage:**
- Health endpoint: Automated test coverage ✅
- Shell scripts: Manual test validation ✅
...
**Gate: PASS** → docs/qa/gates/5.4-monitoring-maintenance-setup.yml
